...
const MODELS = [
  { id: "nano-banana", name: "Nano Banana", description: "Default", maxImages: 4 },
];
...
export function GenerateTab({ initialData, onInitialDataConsumed, onLoad }: GenerateTabProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setModel(MODELS.some((m) => m.id === initialData.model) ? initialData.model : "nano-banana");
    ...
    try {
      ...
      for (const img of response.data) {
        if (img.url) {
          ...
          saveToHistory("generate", img.url, displayPrompt, { model, type, aspectRatio });
        }
      }
  ...
  const handleDownload = async (url: string, index: number) => {
    try {
      ...
      a.href = downloadUrl;
      a.download = `nanobanana-${Date.now()}-${index + 1}.${type === "jpeg" ? "jpg" : type}`;
  ...
  return (
    <div className="grid items-start gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {TYPE_OPTIONS.map((option) => (
                  <OptionCard key={option.value} selected={type === option.value} onClick={() => setType(option.value)}>
      ...
      <div className="space-y-2">
        ...
        {results.length === 0 ? (
          <Card className="flex min-h-[308px] items-center justify-center border-dashed">
